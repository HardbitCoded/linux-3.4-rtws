...
static void set_load_weight(struct task_struct *p)
{
    ...
    //Adaptation from 2.6.36 RTWS
    if(p->policy == SCHED_RTWS){
        load->weight = WEIGHT_RTWS;
        load->inv_weight = WMULT_RTWS;
    }
...
void sched_fork(struct task_struct *p)
{
    ...
    if (unlikely(p->sched_reset_on_fork)) {
        if (task_has_rt_policy(p) || task_has_rtws_policy(p)) {
...
void set_user_nice(struct task_struct *p, long nice)
{
     ...
     * SCHED_FIFO/SCHED_RR:
     */
    if (task_has_rtws_policy(p) || task_has_rt_policy(p)) {
...
recheck:
    ...
    if (param->sched_priority < 0 ||
        ...
        return -EINVAL;
    /*
         * Valid priorities for SCHED_RTWS is MAX_WS_PRIO-1.
         *
         * SCHED_RTWS sets param->sched_priority = 0 and manipulates
         * param_ex for his own definitions. Final prio value is set on
...
init_rtws_global_rq(&rtws_global);
    ...
    for_each_possible_cpu(i) {
        ...
        init_cfs_rq(&rq->cfs);
        init_rt_rq(&rq->rt, rq);
        init_rtws_rq(&rq->rtws, rq, &rtws_global);
#ifdef CONFIG_FAIR_GROUP_SCHED
